...
class GetHandler(BaseHandler):
    ...
    def post(self):
        ...
        if address_ids:
            address_models = self.model_config.filter_all(AddressModel, user_id=self.session['user_id'],
                                                          filters=AddressModel.id.in_(tuple(address_ids)))
        else:
            address_models = self.model_config.all(AddressModel, user_id=self.session['user_id'])
        res = []
