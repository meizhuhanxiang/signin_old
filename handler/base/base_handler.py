...
def handler(fun):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        try:
            if self.get_json_argument('test', False, allow_null=True):
                self.session['user_id'] = 1
                self.session.save()
            if not self.get_json_argument('test', False, allow_null=True) and not self.session.get('user_id', ''):
        ...
        api_logger().info(
            'session_id: %s, %s response: %s' % (
            self.get_secure_cookie("session_id"), self.request.path, re.sub(r'(\\n|\\|\s+)', '', resp)))
        self.write(resp)
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def initialize(self):
        ...
        self.logger.info(
            'session_id: %s, %s request body: %s' % (self.get_secure_cookie("session_id"), self.request.path,
