...
class GetHandler(BaseHandler):
    ...
    def post(self):
        ...
        if not order_ids:
            order_models = self.model_config.all(OrderModel, user_id=self.session['user_id'], status=status)
        else:
            order_models = self.model_config.filter_all(OrderModel, OrderModel.id.in_(tuple(order_ids)), user_id=self.session['user_id'],
                                                        status=status,
                                                        )
        orders = []
        total_price = 0
